...
class CourseBase(BaseModel):
    ...
    def validate_semester(cls, v):
        try:
            ...
            raise ValueError('Semester must be a numeric string between 1 and 12')
        if not (1 <= val <= 12):
    ...
    def validate_name_ar(cls, v):
        if v and not re.search(r'[\u0600-\u06FF]', str(v)):
            raise ValueError('nameAr must contain Arabic characters')
        return v

    @field_validator('name_en', mode='before')
    @classmethod
    def validate_name_en(cls, v):
        if v and re.search(r'[\u0600-\u06FF]', str(v)):
            raise ValueError('nameEn must not contain Arabic characters')
        return v
...
class PlanBase(BaseModel):
    ...
    max_credits_per_semester: int = Field(default=18, gt=0, le=40, alias="maxCreditsPerSemester")
